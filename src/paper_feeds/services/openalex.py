...
def _openalex_get(endpoint: str, params: dict, email: str | None) -> requests.Response:
    """
    ...
    return requests.get(OPENALEX_API_URL + endpoint, params=params, headers=headers)


def get_journal_homepages(issns: list[str], email: str | None) -> dict:
    """
...
def _index_issns(journals: list[str]) -> dict:
    ...
    return {
        issn: journal.get("homepage_url", None)
