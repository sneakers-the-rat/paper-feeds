...
from paper_feeds.const import STATIC_DIR, TEMPLATE_DIR
from paper_feeds.db import create_tables, get_engine
...
@app.post("/search")
async def search(
    request: Request, search: Annotated[str, Form()], background: BackgroundTasks
) -> _TemplateResponse:
    """
    ...
    background.add_task(
        journal_service.get_journal_homepages,
        journals=results,
        email=config.crossref_email,
    )

    return templates.TemplateResponse(
        "partials/feed-list.html", {"request": request, "journals": results}
    )


@app.get("/journals/{issn}")
async def journal_page(request: Request, issn: str) -> _TemplateResponse:
    journal = crossref.load_journal(issn)

    # TODO: Trigger background task to update papers in journal here

    return templates.TemplateResponse(
        "pages/journal.html", {"request": request, "feed_id": issn, "journal": journal}
    )


@app.post("/journals/{issn}/feed")
async def make_feed(
    feed_id: Annotated[str, Form()], request: Request, background: BackgroundTasks
) -> Optional[_TemplateResponse]:
    """
    Enable the "feed" attribute for the given journal and trigger an initial population

    Don't expose the full journal model to CRUD, the only field we want to be able to
    update is `feed`, and the only thing we want to do to it is set it to ``True``
    """
    with Session(engine) as session:
        statement = (
            select(models.Journal).join(models.ISSN).where(models.ISSN.value == feed_id)
        )
...
async def journal_papers(
    request: Request, issn: str, rows: int = Query(default=40, le=200), offset: int = 0
) -> Sequence[PaperRead] | _TemplateResponse:
    engine = get_engine()
    with Session(engine) as session:
        # get journal
        journal = crossref.load_journal(issn)
    ...
    if request.headers.get("content-type", False) == "application/json":
        ...
        return templates.TemplateResponse(
            "partials/paper-list.html", {"request": request, "papers": papers}
        )
